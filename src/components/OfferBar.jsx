...
/* `quiet` renders the same offer and the same WhatsApp link with no pill
   badge and no button shape: the claim becomes an underlined text link. */
const OfferBar = ({ quiet = false }) => {
  ...
  return (
    <div
      className={quiet ? 'offer-bar offer-bar-quiet' : 'offer-bar'}
      role="region"
      ...
      <div className="offer-bar-inner">
        ...
        {isFormPage ? (
          ...
          >
            ...
            {quiet ? 'Claim on WhatsApp' : 'WhatsApp Now'}
          </a>
        )}
      </div>
    </div>
