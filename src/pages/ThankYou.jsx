...
// then fall back to the customer details Zoho Bookings appends to its redirect,
// in case this page is also used as a post-booking landing page.
const readCustomerEmail = () => {
  try {
    ...
    // sessionStorage blocked (private mode) — fall through to the booking params.
  }
  return readBookedEmail();
};

const ThankYou = () => {
  // Read during the first render, which is both before the sessionStorage
  // cleanup in the effect below wipes the value and early enough that the
  // personalized copy is in the first paint.
  const [customerEmail] = useState(readCustomerEmail);
